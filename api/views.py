...
from .models import User, Deer, UserRecords
...
class CalculateChargeView(View):
    ...
    def post(self, request):
        ...
        UserRecords.objects.create(user_id=user.id, deer_id=deer.id, start_at=userdto.use_start_at,
                                   end_at=userdto.use_end_at,
                                   end_lat_lng=Point(userdto.use_end_lat, userdto.use_end_lng))

        return JsonResponse(
            {'message': 'SUCCESS', 'charge_cost': f'{policies_and_charge[-1]}, "요금 근거: f{policies_and_charge[:-1]}'},
            status=200)
